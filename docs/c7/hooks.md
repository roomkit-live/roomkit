...
| `ON_AI_RESPONSE` | ASYNC | `(AIResponseEvent, ctx) -> None` | AI generation completed. Carries the turn's text — `response_content`, the segments a tool call cut joined with a blank line, and `segments` one by one — plus usage, latency, tool call count, and `loop_end_reason` — which of the tool loop's rules ended the turn (`completed`, `max_rounds`, `timeout`, `cancelled`, `force_stopped`, `truncated`, `empty_response`). Read that rather than inferring a cut-off from `tool_calls_count`, which counts the calls the turn ran. |
