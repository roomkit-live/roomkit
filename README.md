...
pip install roomkit[sip]               # SIP voice + video backend
pip install roomkit[rtp]               # RTP voice + video backend
pip install roomkit[local-audio]       # Local mic/speaker backend

# Video backends
pip install roomkit[local-video]       # Webcam capture (OpenCV)
pip install roomkit[screen-capture]    # Screen capture
pip install roomkit[video]             # A/V recording (PyAV, VP9/H.264)
...
| **Video** | `video` | video | Real-time video (webcam, RTP, screen capture) |
| **Audio+Video** | `av` | audio, video | Combined SIP A/V calls (VP9/H.264 + audio) |
...
| `RTPVoiceBackend` | Raw RTP audio transport | `roomkit[rtp]` |
...
RoomKit supports real-time video over SIP/RTP with VP9, H.264, and VP8 codecs, plus room-level audio+video recording to MP4:

```python
from roomkit import AudioVideoChannel, RoomKit
from roomkit.recorder.pyav import PyAVMediaRecorder
from roomkit.recorder.base import MediaRecordingConfig, RoomRecorderBinding, ChannelRecordingConfig
from roomkit.video.backends.sip import SIPVideoBackend

kit = RoomKit()

# Combined audio+video SIP backend
...
The recorder handles VP9 → H.264 transcoding, per-track A/V sync, and proper MP4 finalization. Vision AI providers (OpenAI, Gemini) can analyze video frames and inject descriptions into the AI conversation context.
...
src/roomkit/
  ...
  video/           Video subsystem
    backends/        Video transports (RTP, SIP, Local, Screen)
    vision/          Vision AI (OpenAI, Gemini)
    recorder/        Video recording (OpenCV, PyAV)
  recorder/        Room-level A/V recording (PyAV muxer)
