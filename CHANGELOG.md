...
- **`kit.join()` / `kit.leave()`** — unified session lifecycle API. `join(room_id, channel_id)` creates and starts a session (pull model); `join(room_id, channel_id, session=session)` binds an externally-created session (push model, e.g. SIP). `leave(session)` stops, unbinds, and disconnects. Replaces `connect_voice`, `disconnect_voice`, `connect_video`, `disconnect_video`, and `bind_voice_session` (now deprecated wrappers).
