...
# Vendored from gradio-app/fastrtc@v0.0.34 (Apache-2.0); see LICENSE in this directory.
...
Track = (
    VideoCallback | VideoStreamHandler_ | AudioCallback | ServerToClientAudio | ServerToClientVideo
)
...
class WebRTCConnectionMixin:
    ...
    async def connection_timeout(
        ...
        try:
            await asyncio.wait_for(self.connection_timeouts[webrtc_id].wait(), time_limit)
    ...
    async def update_connection(
        ...
        if webrtc_id in self.connections:
            self.data_channels[webrtc_id].send(create_message("update_connection", desired_state))
    ...
    async def output_stream(self, webrtc_id: str) -> AsyncGenerator[AdditionalOutputs, None]:
    ...
    def set_additional_outputs(self, webrtc_id: str) -> Callable[[AdditionalOutputs], None]:
    ...
    async def _trigger_response(self, webrtc_id: str, args: list[Any] | None = None):
        ...
        # so no connection here is a ReplyOnPause — this manual trigger is inert.
    ...
    async def handle_offer(self, body, set_outputs):
        ...
        if body.get("type") == "ice-candidate" and "candidate" in body:
            ...
            if webrtc_id not in self.pcs:
                logger.warning(f"Received ICE candidate for unknown connection: {webrtc_id}")
                return JSONResponse(
                    status_code=200,
                    content={
                        "status": "failed",
            ...
            if pc.connectionState != "closed":
                try:
                    ...
                    # A minimal host candidate is 8 tokens (foundation, component,
                    # protocol, priority, ip, port, "typ", type). Upstream required
                    # >=10, which only Chrome's optional extension attrs satisfy —
                    # Safari/WebKit emit exactly 8 and every candidate was rejected
                    # ("Invalid candidate format"). Accept >=8.
        ...
        def _(channel):
            ...
            def _(message):
                ...
                if channel.readyState == "open":
                    channel.send(create_message("log", data=f"Server received: {message}"))
