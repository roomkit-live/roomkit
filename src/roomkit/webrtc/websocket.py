...
class WebSocketHandler:
    def __init__(
        ...
        additional_outputs_factory: Callable[[str], Callable[[AdditionalOutputs], None]],
    ):
        ...
        try:
            while not self.quit.is_set():
                ...
                if message["event"] == "media":
                    ...
                    audio_array = np.frombuffer(audioop.ulaw2lin(audio_payload, 2), dtype=np.int16)
                    ...
                    self.websocket = websocket
                    current_context.set(Context(webrtc_id=self.stream_id, websocket=websocket))
                    if isinstance(self.stream_handler, AsyncStreamHandler):
    ...
    def emit_with_context(self):
        current_context.set(Context(webrtc_id=cast(str, self.stream_id), websocket=self.websocket))
    ...
    async def _wait_for_audio_completion(self):
        ...
        try:
            ...
            if total_wait > 0:
                logger.debug(f"Waiting {total_wait:.2f}s for audio to complete before closing")
