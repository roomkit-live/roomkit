...
# directory. The WebRTC data models use plain pydantic bases instead of
# gradio.data_classes so this module carries no gradio dependency.
import asyncio
import fractions
import functools
import inspect
...
current_channel: ContextVar[DataChannel | None] = ContextVar("current_channel", default=None)
...
async def player_worker_decode(
    ...
    while not thread_quit.is_set():
        try:
            # Get next frame
            frame, outputs = split_output(await asyncio.wait_for(next_frame(), timeout=60))
            if (
            ...
            logger.warning("Timeout in frame processing cycle after %s seconds - resetting", 60)
            continue
