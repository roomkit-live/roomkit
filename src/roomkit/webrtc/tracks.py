    ...
    async def recv(self):  # type: ignore
        ...
        try:
            ...
            if isinstance(outputs, CloseStream):
                cast(DataChannel, self.channel).send(create_message("end_stream", outputs.msg))
                self.stop()
                return None
            if (
...
HandlerType = (
    StreamHandlerImpl | VideoStreamHandlerImpl | VideoEventHandler | Callable | VideoStreamHandler
)
    ...
    async def start(self):
        if not self.has_started:
            ...
            if isinstance(self.event_handler, AsyncHandler):
                ...
                callable = functools.partial(loop.run_in_executor, None, self.event_handler_emit)
    ...
    async def recv(self):  # type: ignore
        try:
            ...
            if isinstance(frame, CloseStream):
                cast(DataChannel, self.channel).send(create_message("end_stream", frame.msg))
                self.stop()
                return
            ...
            if time.time() - self.last_timestamp > 10 * (
                self.event_handler.output_frame_size / self.event_handler.output_sample_rate
            ):
                self._start = None
    ...
    async def recv(self):  # type: ignore
        try:
            ...
            if isinstance(data, CloseStream):
                cast(DataChannel, self.channel).send(create_message("end_stream", data.msg))
                self.stop()
                return
            if data is None:
                self.stop()
